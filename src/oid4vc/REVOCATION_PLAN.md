...
**Status:** All phases complete and gathered on the integration branch. One PR
remains — see *Branch and PR structure* below.
...
The work now ships as a single pull request. The **integration branch** holds
every phase, and merges to `master` once.

```
feat/credentials-status-and-revoke      PR 1 — Phases 0-6  (integration branch)
        │ merges into
        ▼
master
```
...
| PR | Branch | Owns | Base |
| --- | --- | --- | --- |
| 1 | `feat/credentials-status-and-revoke` | Phases 0-6, and this document | `master` |

**This was a stack of three.** `feat/credentials-status-local-resolve` (PR 3,
Phases 4 and 6) was fast-forwarded into `feat/credentials-status-endpoint`
(PR 2, Phases 3, 5b, 5e), and that in turn into the integration branch. All
three branches ended up at the same commit, so PR 2 and PR 3 both have empty
diffs — close them rather than trying to merge them. Phase headings below
still name the PR that originally owned each phase.
...
Running total: after Phase 4, **27 suites, 237 tests** (`yarn test`) plus
...
Phase 2 added 6, Phase 3 added 7 unit + 2 e2e, Phase 4 added 12, the
compare-and-set and multi-entry work added 9.)
...
above: the integration branch carries both phases before it reaches `master`,
so no deployable branch ever issues credentials pointing at a URL nothing
serves.
...
- Everything else is settled. Note that Phase 2 is the point of no return:
  ...
  load-bearing forever (§8) — which is why Phase 3 has to reach `master` in
  the same merge.
